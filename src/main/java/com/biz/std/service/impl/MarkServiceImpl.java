...
public class MarkServiceImpl implements MarkService {
    ...
    @Override
    @Transactional
    public void studentAddSubject(MarkVO markVO) {
        SubjectVO subjectVO = POToVOUtils.subjectPOToVO(subjectJpaRepository.findOne(markVO.getSubjectVO().getId()));
        int newStudentNumber = subjectVO.getStudentNumber() + 1;
        float newAvgScore = (subjectVO.getAvgScore() * subjectVO.getStudentNumber() + subjectVO.getAvgScore()) / newStudentNumber;
        subjectJpaRepository.updateStudentNumberAndAvgScore(newStudentNumber, newAvgScore, markVO.getSubjectVO().getId());//更新选课人数和平均分
        markJpaRepository.save(VOToPOUtils.markPOVOToPO(markVO));//存选课
    }
